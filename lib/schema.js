...
export function generateBreadcrumbSchema(category, slug, title) {
  const items = [
    {
      ...
      "position": 1,
      "name": "Home",
      "item": "https://basicgermany.com"
    },
    {
      "@type": "ListItem",
      "position": 2,
      "name": "Knowledge",
      "item": "https://basicgermany.com/knowledge"
    }
  ];
  ...
  if (category) {
    items.push({
      ...
      "item": `https://basicgermany.com/knowledge?category=${encodeURIComponent(category)}`
    });
  }
  
  // Add current page (position adjusts based on whether category exists)
  items.push({
    "@type": "ListItem",
    "position": items.length + 1,
    "name": sanitizeForJSON(title),
    "item": `https://basicgermany.com/knowledge/${slug}`
  });
