...
export function createProductCardHTML(product, bgColor) {
  ...
  const descHtml = product.description 
    ? `<div style="font-size:14px;color:rgba(0,0,0,0.7);line-height:1.4;margin:8px 0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Oxygen,Ubuntu,Cantarell,sans-serif">${product.description}</div>` 
    : '';
...
export function createAdCardHTML(params) {
  ...
  const descHtml = description 
    ? `<div style="font-size:14px;color:rgba(0,0,0,0.7);line-height:1.4;margin:8px 0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Oxygen,Ubuntu,Cantarell,sans-serif">${description}</div>` 
    : '';
