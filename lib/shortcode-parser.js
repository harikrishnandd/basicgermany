...
export function createAdCardHTML(params) {
  ...
  return `<div style="background:${bg};border-radius:12px;padding:20px;margin:24px 0;border:1px solid var(--borderColor, #e0e0e0);text-align:center">${logoHtml}<div style="font-size:20px;font-weight:700;color:var(--systemPrimary);margin-bottom:4px">${name}</div>${priceHtml}<button style="width:100%;padding:12px;background:var(--keyColor, #007AFF);color:white;border:none;border-radius:8px;font-size:16px;font-weight:600;cursor:pointer;margin-top:16px" onclick="window.open('${link || '#'}','_blank')">Get</button></div>`;
}
