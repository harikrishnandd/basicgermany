...
export function createProductCardHTML(product, bgColor) {
  ...
  // Icon/image with theme-aware background and border
  const logoHtml = isIcon
    ? `<div style="width:64px;height:64px;border-radius:12px;background:var(--systemQuaternary,rgba(120,120,128,0.16));display:flex;align-items:center;justify-content:center;flex-shrink:0;border:1px solid var(--separatorNonOpaque,rgba(60,60,67,0.29))"><span class="material-symbols-outlined" style="font-size:36px;color:var(--keyColor,#007AFF)">${product.logo}</span></div>`
    : `<div style="width:64px;height:64px;border-radius:12px;overflow:hidden;flex-shrink:0;display:flex;align-items:center;justify-content:center;border:1px solid var(--separatorNonOpaque,rgba(60,60,67,0.29))"><img src="${product.logo}" alt="${product.name}" style="width:100%;height:100%;object-fit:contain"/></div>`;

  // Capsule price pill - reduced height by 20% (padding 4px -> 3px)
  const priceHtml = product.free 
    ? `<span style="display:inline-block;padding:3px 10px;background:#34c759;color:white;border-radius:12px;font-size:13px;font-weight:600;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Oxygen,Ubuntu,Cantarell,sans-serif">Free</span>`
    : `<span style="display:inline-block;padding:3px 10px;background:var(--systemQuaternary,rgba(120,120,128,0.16));color:var(--systemPrimary,#000);border-radius:12px;font-size:13px;font-weight:600;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Oxygen,Ubuntu,Cantarell,sans-serif"><span class="material-symbols-outlined" style="font-size:14px;vertical-align:middle;margin-right:2px">${product.currency || 'euro_symbol'}</span>${product.price}</span>`;
...
export function createAdCardHTML(params) {
  ...
  const logoHtml = image
    ? `<div style="width:64px;height:64px;border-radius:12px;overflow:hidden;flex-shrink:0;display:flex;align-items:center;justify-content:center;border:1px solid var(--separatorNonOpaque,rgba(60,60,67,0.29))"><img src="${image}" alt="${name}" style="width:100%;height:100%;object-fit:contain"/></div>`
    : logo
    ? `<div style="width:64px;height:64px;border-radius:12px;background:var(--systemQuaternary,rgba(120,120,128,0.16));display:flex;align-items:center;justify-content:center;flex-shrink:0;border:1px solid var(--separatorNonOpaque,rgba(60,60,67,0.29))"><span class="material-symbols-outlined" style="font-size:36px;color:var(--keyColor,#007AFF)">${logo}</span></div>`
    : `<div style="width:64px;height:64px;border-radius:12px;background:var(--systemQuaternary,rgba(120,120,128,0.16));display:flex;align-items:center;justify-content:center;flex-shrink:0;border:1px solid var(--separatorNonOpaque,rgba(60,60,67,0.29))"><span class="material-symbols-outlined" style="font-size:36px;color:var(--systemSecondary)">image</span></div>`;

  // Capsule price pill - reduced height by 20% (padding 4px -> 3px)
  const priceHtml = (price && price !== '0')
    ? `<span style="display:inline-block;padding:3px 10px;background:var(--systemQuaternary,rgba(120,120,128,0.16));color:var(--systemPrimary,#000);border-radius:8px;font-size:13px;font-weight:600;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Oxygen,Ubuntu,Cantarell,sans-serif"><span class="material-symbols-outlined" style="font-size:14px;vertical-align:middle;margin-right:2px">euro_symbol</span>${price}</span>`
    : `<span style="display:inline-block;padding:3px 10px;background:#34c759;color:white;border-radius:12px;font-size:13px;font-weight:600;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Oxygen,Ubuntu,Cantarell,sans-serif">Free</span>`;
