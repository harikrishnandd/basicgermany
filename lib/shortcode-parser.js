...
export function createProductCardHTML(product, bgColor) {
  ...
  const priceDisplay = product.free ? 
    `<span style="
      ...
      font-weight: 500;
    ">
      <span className="material-symbols-outlined" style="font-size: 14px;">check_circle</span>
      Free
    </span>` : 
    `<div style="display: flex; align-items: center; gap: 4px;">
      <span className="material-symbols-outlined" style="font-size: 16px; color: var(--systemSecondary);">
  ...
  " onclick="window.open('${product.link || '#'}', '_blank')">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px;">
      ...
      ">
        ${isIconName ? 
          `<span className="material-symbols-outlined" style="font-size: 32px; color: var(--keyColor);">
            ${product.logo}
          </span>` :
...
export function createAdCardHTML(params) {
  ...
  const isIconName = logo && typeof logo === 'string' && !logo.startsWith('http');
