...
export function createProductCardHTML(product, bgColor) {
  ...
  const logoHtml = isIcon
    ...
    : `<div style="width:100%;aspect-ratio:16/9;max-height:200px;border-radius:12px;overflow:hidden;margin-bottom:16px;background:var(--systemSenary, #f5f5f5);display:flex;align-items:center;justify-content:center"><img src="${product.logo}" alt="${product.name}" style="width:100%;height:100%;object-fit:contain"/></div>`;
  ...
  return `<div style="background:${bgColor};border-radius:12px;padding:20px;margin:24px 0;border:1px solid var(--borderColor, #e0e0e0);text-align:center;transition:transform 0.2s ease,box-shadow 0.2s ease" onmouseover="this.style.transform='translateY(-2px)';this.style.boxShadow='0 8px 16px rgba(0,0,0,0.1)'" onmouseout="this.style.transform='translateY(0)';this.style.boxShadow='none'">${logoHtml}<div style="font-size:20px;font-weight:700;color:var(--systemPrimary);margin-bottom:4px">${product.name}</div>${priceHtml}${descHtml}<button style="width:100%;padding:12px;background:var(--keyColor, #007AFF);color:white;border:none;border-radius:8px;font-size:16px;font-weight:600;cursor:pointer;margin-top:16px;transition:opacity 0.2s" onclick="window.open('${product.link || '#'}','_blank')" onmouseover="this.style.opacity='0.9'" onmouseout="this.style.opacity='1'">Get</button></div>`;
}
...
export function createAdCardHTML(params) {
  const { name, price, link, logo, image, bg } = params;
  ...
  const logoHtml = image
    ? `<div style="width:100%;aspect-ratio:16/9;max-height:200px;border-radius:12px;overflow:hidden;margin-bottom:16px;background:var(--systemSenary, #f5f5f5);display:flex;align-items:center;justify-content:center"><img src="${image}" alt="${name}" style="width:100%;height:100%;object-fit:contain"/></div>`
    : logo
  ...
  return `<div style="background:${bg};border-radius:12px;padding:20px;margin:24px 0;border:1px solid var(--borderColor, #e0e0e0);text-align:center;transition:transform 0.2s ease,box-shadow 0.2s ease" onmouseover="this.style.transform='translateY(-2px)';this.style.boxShadow='0 8px 16px rgba(0,0,0,0.1)'" onmouseout="this.style.transform='translateY(0)';this.style.boxShadow='none'">${logoHtml}<div style="font-size:20px;font-weight:700;color:var(--systemPrimary);margin-bottom:4px">${name}</div>${priceHtml}<button style="width:100%;padding:12px;background:var(--keyColor, #007AFF);color:white;border:none;border-radius:8px;font-size:16px;font-weight:600;cursor:pointer;margin-top:16px;transition:opacity 0.2s" onclick="window.open('${link || '#'}','_blank')" onmouseover="this.style.opacity='0.9'" onmouseout="this.style.opacity='1'">Get</button></div>`;
}
