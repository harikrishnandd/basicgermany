...
export async function createBlogArticle(articleData) {
  try {
    ...
    if (articleData.category) {
      await syncGlobalCategory(null, articleData.category, 'knowledge');
    }
...
export async function updateBlogArticle(articleId, updates) {
  try {
    ...
    // Get current article data to check for category change
    const docSnap = await getDoc(docRef);
    const oldCategory = docSnap.exists() ? docSnap.data().category : null;
    ...
    // Sync global categories if category changed
    if (updates.category && updates.category !== oldCategory) {
      await syncGlobalCategory(oldCategory, updates.category, 'knowledge');
    }
  } catch (error) {
...
export async function deleteBlogArticle(articleId) {
  try {
    ...
    // Get article data before deletion for category sync
    const docSnap = await getDoc(docRef);
    const category = docSnap.exists() ? docSnap.data().category : null;
    
    await deleteDoc(docRef);

    // Sync global categories
    if (category) {
      await syncGlobalCategoryOnDelete(category, 'knowledge');
    }
  } catch (error) {
